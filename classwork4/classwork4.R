#Загрузите данные в датафрейм.  
gmp <- read.table("https://raw.githubusercontent.com/SergeyMirvoda/MD-DA-2018/master/data/gmp.dat")
gmp$pop<-gmp$gmp/gmp$pcgmp
estimate.scaling.exponent <- function(a, y0=6611, response=gmp$pcgmp,
                                      predictor = gmp$pop, maximum.iterations=100, deriv.step = 1/100,
                                      step.scale = 1e-12, stopping.deriv = 1/100) {
  mse <- function(a) { mean((response - y0*predictor^a)^2) }
  for (iteration in 1:maximum.iterations) {
    deriv <- (mse(a+deriv.step) - mse(a))/deriv.step
    a <- a - step.scale*deriv
    if (abs(deriv) <= stopping.deriv) { break() }
  }
  fit <- list(a=a,iterations=iteration,
              converged=(iteration < maximum.iterations))
  return(fit)
}
#Пример вызова с начальным значением a
before<-estimate.scaling.exponent(0.15)


#С помошью полученного коэффициента постройте кривую (функция curve) зависимости
curve((y0=6611)*x^estimate.scaling.exponent(0.15)$a,gmp$pop, xlab="Население", ylab="Доход на душу населения ($/человеко-год)", main="Метрополии США, 2006")


#Удалите точку из набора исходных данных случайным образом, как изменилось статистическая оценка коэффициента a?
randomRow<-round(runif(1,max=367))
gmp<-gmp[-randomRow,]
after<-estimate.scaling.exponent(0.15)
after
before$a-after$a
#Вывод: Изменения a несущественны. Примерно (-2.186975e-05). 


#Запустите оценку несколько раз с разных стартовых точек. Как изменилось значение a?
estimate.scaling.exponent(0.1)
estimate.scaling.exponent(0.13)
estimate.scaling.exponent(0.17)
estimate.scaling.exponent(0.20)
estimate.scaling.exponent(0.25)
estimate.scaling.exponent(0.30)
estimate.scaling.exponent(0.33)
#Вывод: при значениях [0.1 0.20] коэффициент а не меняется, 
# а количество итерации тем меньше, чем ближе начальное значение а к исходному значению
# При начальном значении 0.25 сходимости нет
# При начальных занчения 0.30 и 0.35 значение a различно.

